...
class LatentSpacePolicy(BasePolicy):
    ...
    def actions_np_ours(self, conditions):
        latents = self.latents_model.predict(conditions)
        if self._deterministic:
            return self.deterministic_actions_model.predict(conditions), latents

        return self.actions_model_for_fixed_latents.predict(
            [*conditions, latents]), latents
