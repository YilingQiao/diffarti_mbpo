...
class GetCOM(torch.autograd.Function):
	@staticmethod
	def forward(ctx, q, world):
		ctx.save_for_backward(q)
		ans = pd.forward_get_com(q.detach().numpy(), world)
		ctx.world = world
		ans = torch.tensor(ans, dtype=torch.float32)
		return ans
	...
	def backward(ctx, dldans):
		q, = ctx.saved_tensors
		ans = pd.backward_get_com(q.detach().numpy(), dldans.detach().numpy(), ctx.world)
		ans = torch.tensor(ans, dtype=torch.float32)
		return ans, None

get_com = GetCOM.apply
