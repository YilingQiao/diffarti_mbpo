...
class GLearn(RLAlgorithm):
    ...
    def __init__(
        ...
        super(GLearn, self).__init__(**kwargs)
    ...
    def _train(self):
        ...
        """
        if False:
            print(self._eval_deterministic)
            print(self._policy)
            evaluation_environment = self._evaluation_environment
            evaluation_environment._env.env.output_file = '/homes/williamljb/objoutput_pendulum/{}/'.format(self.output_name)
            policy = self._policy
            self.sampler._max_path_length = 100 # for pendulum
            self._eval_n_episodes = 1 # for pendulum
            evaluation_paths = self._evaluation_paths(
                policy, evaluation_environment)
            evaluation_metrics = self._evaluate_rollouts(
                evaluation_paths, evaluation_environment)
            print('total: ',evaluation_metrics['return-average'])
            evaluation_environment._env.env.reset()
            while True:
                continue
