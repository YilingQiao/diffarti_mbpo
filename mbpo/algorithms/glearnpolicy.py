...
class GLearnPolicy(RLAlgorithm):
    ...
    def __init__(
        ...
        super(GLearnPolicy, self).__init__(**kwargs)
    ...
    def _initial_exploration_hook(self, env, initial_exploration_policy, pool):
        ...
        while self.sampler._total_samples < self._n_initial_exploration_steps:
            self.sampler.sample(self._num_ext, self._maxdelta)
    ...
    def _train(self):
        ...
        if False:
            ...
            evaluation_environment._env.env.output_file = '/homes/williamljb/mbpo_ant_output/'
            policy = self._policy
        ...
        for self._epoch in gt.timed_for(range(self._epoch, self._n_epochs)):
            ...
            self._training_progress = Silent() #Progress(self._epoch_length * self._n_train_repeat)
    ...
    def _init_actor_update(self):
        ...
        actions0 = self._policy.actions([self._observations_ph])
        log_pis0 = self._policy.log_pis([self._observations_ph], actions0)

        assert log_pis0.shape.as_list() == [None, 1]
        ...
        if isinstance(self._target_entropy, Number):
            alpha_loss = -tf.reduce_mean(
                log_alpha * tf.stop_gradient(log_pis0 + self._target_entropy))
        ...
        Q_log_targets0 = tuple(
            Q([self._observations_ph, actions0])
            for Q in self._Qs)
        min_Q_log_target0 = tf.reduce_min(Q_log_targets0, axis=0)

        policy_kl_losses0 = (
                alpha * log_pis0
                - min_Q_log_target0)

        assert policy_kl_losses0.shape.as_list() == [None, 1]

        actions = self._policy.actions_replay([self._observations_ph, self._latents_ph])
        ...
        # importance = tf.stop_gradient(tf.exp(-1.*tf.abs(actions-self._actions_ph)))
        # importance = tf.expand_dims(importance, axis=1)
        importance = tf.stop_gradient(tf.exp(-3*tf.linalg.norm(actions-self._actions_ph, axis=-1, keepdims=True)))
        jacvs = tf.reshape(tf.gradients(self._get_Q_next_obs(), self._next_observations_ph), 
            [-1, self._observation_shape[0]]) # bxo
        jacvs = tf.batch_gather(jacvs, tf.reshape(self._jacind_ph, [-1,1])) # bx1
        # jacvs = tf.gather(jacvs, tf.reshape(self._jacind_ph, [-1,1]), axis=1, batch_dims=1) # bx1
        ...
        # fake_loss = tf.linalg.matvec((self._jacr_ph + losstd) * importance, actions)
        fake_loss = tf.linalg.matvec(self._jacr_ph + losstd, actions)
        fake_loss = importance * fake_loss
        ...
        policy_loss = tf.reduce_mean(policy_kl_losses + policy_kl_losses0)
